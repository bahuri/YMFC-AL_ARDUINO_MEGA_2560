...
void esc_pulse_output(){
  zero_timer = micros();
  PORTF |= B11110000;                                            //Set port 4, 5, 6 and 7 high at once
  ...
  while(PORTF >= 16){                                            //Execute the loop until digital port 4 to 7 is low.
    esc_loop_timer = micros();                                   //Check the current time.
    if(timer_channel_1 <= esc_loop_timer)PORTF &= B11101111;     //When the delay time is expired, digital port 4 is set low.
    if(timer_channel_2 <= esc_loop_timer)PORTF &= B11011111;     //When the delay time is expired, digital port 5 is set low.
    if(timer_channel_3 <= esc_loop_timer)PORTF &= B10111111;     //When the delay time is expired, digital port 6 is set low.
    if(timer_channel_4 <= esc_loop_timer)PORTF &= B01111111;     //When the delay time is expired, digital port 7 is set low.
  }
}
